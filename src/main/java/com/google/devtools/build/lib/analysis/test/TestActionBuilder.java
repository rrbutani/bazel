...
public final class TestActionBuilder {
  ...
  private TestParams createTestAction()
    ...
    inputsBuilder.add(testActionFilesToRun.getExecutable());
    if (testActionFilesToRun.getRunfilesSupport() != null) {
      ...
      inputsBuilder.addTransitive(testActionFilesToRun.getRunfilesSupport().getRunfiles().getAllArtifacts());
...
/*     inputsBuilder.addTransitive(testActionXmlGeneratorFilesToRun.getFilesToRun());
    ...
    NestedSetBuilder<Artifact> xmlGeneratorFilesToRun = NestedSetBuilder.compileOrder();
    {
      xmlGeneratorFilesToRun.add(testActionXmlGeneratorFilesToRun.getExecutable());
      if (testActionXmlGeneratorFilesToRun.getRunfilesSupport() != null) {
        ...
        xmlGeneratorFilesToRun.addTransitive(testActionXmlGeneratorFilesToRun.getRunfilesSupport().getRunfiles().getAllArtifacts());
      }
      ...
      // inputsBuilder.addTransitive(xmlGeneratorRunfilesSupplier.getAllArtifacts());
    }
    ...
    // ))
    RunfilesSupplier runfilesSupplier = CompositeRunfilesSupplier.of(
      runfilesSupport, testActionRunfilesSupplier
    );
      ...
      for (int run = 0; run < runsPerTest; run++) {
        ...
        TestRunnerAction testRunnerAction =
            new TestRunnerAction(
                actionOwner,
                inputs,
                null, // runfilesSupport.getRunfilesMiddleman(), // should be unused...
                runfilesSupplier, // runfilesSupport,
                ...
                lcovMergerRunfilesMiddleman,
                // TODO(rrbutani, rebase): replicate the lcov changes to the xmlGenerator? (i.e. pass runfiles middlemen instead)
